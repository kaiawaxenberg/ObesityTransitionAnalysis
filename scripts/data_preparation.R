...
################################################################################
# Load HPACC survey data and combine datasets
################################################################################

## Load raw data tables and select variables of interest
HPACC_LMIC1 <- read.csv("data/HPACC/HPACC_LMIC_Pt1_2025-07-15.csv")
HPACC1 <- HPACC_LMIC1 |>
  dplyr::select(
    Country, sex, marital, edyears, rural, age, bmi, bmicat, wealth_quintile,
...
## Add population data to HIC dataset which doesn't include it
...
## Combine data tables and clean up
...
## Save combined HPACC data to save time later
save.image("HPACC.RData")
## Load HPACC data if returning to this script
load("Rdata/HPACC.RData")

################################################################################
# Load cleaned EHIS and ISSP survey data and combine with HPACC
################################################################################

## Load cleaned ISSP and EHIS datasets and add pregnancy column
...
## Calorie access from the FAO food balance sheets
food_access <- read.csv("data/daily-per-capita-caloric-supply/daily-per-capita-caloric-supply.csv") |>
  filter(Code != "")

## Income shares from the World Development Indicator database
...
################################################################################
# Join national indicators to survey data
################################################################################

## Function for joining national indicators from nearest year
...
## Join survey data and national data
...
rm(food_access, gdpPcData, incomeShareData, region, nearest_year_join)

################################################################################
# Clean up dataset and fix survey inconsistencies
################################################################################

model_data_1 <- model_data_0 |>
  select(w2, Country, Iso3, bmi, bmicat, dataYear, pregnant,
    wealth_quintile, age, sex, gdpPc, rural,
    kcalSupply = Daily.calorie.supply.per.person,
    incomeShare, population2015, dataset, Region
  ) |>
  mutate(
    ## Harmonise rural metric across surveys
    rural = case_when(
      dataset == "EHIS" ~ case_when(
        rural %in% c("3", "2") ~ "Rural",
        rural %in% c("1") ~ "Urban"
      ),
      dataset == "ISSP" ~ case_when(
        rural %in% c("4", "5", "3") ~ "Rural",
    ...
    ## Clean multiple codes for missing data and replace missing values with NA
    ...
    ## Add binary obesity and overweight values
    obesity = factor(BMI_Category == "Obesity"),
    overweight = factor(BMI_Category %in% c("Overweight", "Obesity")),
    ## Combine underweight and healthy weight into a single category
...
################################################################################
# Omit surveys with missing variables of interest
################################################################################
...
## Remove surveys which completely omit variables of interest
sample_data_1 <- model_data_1 |>
  filter(!paste0(Iso3, dataset) %in% structural_missing$code) |>
  droplevels()

################################################################################
# Generate final parameterisation data
################################################################################
...
sample_data <- sample_data_1 |>
  ...
  ## Scale continuous variables using z-scores for stable parameterisation
  mutate(
    across(
      c(age, kcalSupply, incomeShare, income_ratio, logGdpPc),
      ~ as.numeric(.x)
    ),
    across(c(age, income_ratio),
      ~ as.numeric(scale(.x)),
      .names = "{.col}_z"
    ),
    across(c(kcalSupply, incomeShare, logGdpPc, dataYear),
...
## Save scaling parameters to convert z-scores back to raw values
scaling_params_country <- sample_data |>
  rename(incomeRatio = income_ratio) |>
  summarise(across(
    c(kcalSupply, incomeShare, logGdpPc, dataYear),
    list(
      mean = ~ mean(unique(as.numeric(.x)), na.rm = T),
...
## Choose which countries to keep, removing duplicates where countries appear
## in multiple datasets following this preference order: EHIS, HPACC, ISSP
...
## Calculate n countries per income group in final data
...
## remove duplicated country data from the pooled dataset
...
## Summarise the data by country (Supplementary Table 1)
countryMapping <- read.csv("data/countryMapping.csv")
summary_table <- sample_data_1 |>
...
## Summarise the data by country income group (Table 1)
...
## Calculate Obesity and Overweight prevalence by country for results text
weightedPrev <- sample_data |>
  filter(pooledData) |>
  group_by(Iso3) |>
  mutate(
    obesity = BMI_Category == "Obesity",
...
## Compute the average GDPpc and survey year across the sample for results text
gdp <- sample_data |>
  filter(pooledData) |>
  group_by(Country) |>
...
year <- sample_data |>
  filter(pooledData) |>
  group_by(Country) |>
  summarise(year = mean(dataYear))
median(year$year)

################################################################################
# Output final workspace image with final sample
################################################################################

## Remove tables which don't be used in subsequent analysis
rm(
  structural_missing, model_data_1,
  incomeGroups, summary, weightedPrev,
...
## Save workspace image for use in multinomials.R
