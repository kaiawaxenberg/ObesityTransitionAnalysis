...
## Match country names to mapping data in loaded package
...
################################################################################
## Create functions for weighted summary statistics
################################################################################
...
################################################################################
## BMI summary statistics (for results text)
################################################################################

## Summarise total obesity and overweight prevalence by survey dataset
summary <- map_data |>
...
## Summarise total obesity and overweight prevalence by country
...
################################################################################
## Plot GDP per capita across the datasets (Supplementary Figure 2)
################################################################################
...
################################################################################
## Maps of data coverage (Figure 1 and Supplementary Figure 3)
################################################################################

## Load data for global mapping
...
## Plot data coverage including only surveys with urbanicity
subsample <- map_data |>
  filter(insample_rural) |>
...
################################################################################
## Maps of obesity and overweight prevalence (Supplementary Figure 5)
################################################################################
...
################################################################################
## BMI Barbell plot (Supplementary Figure 4)
################################################################################
...
################################################################################
## Obesity and Overweight boxplots by income quintile
...
quintileBoxplotObesity <- ggplot(
  plot_data,
  aes(x = wealth_quintile, y = obesity_prevalence, fill = gdpLevel)
) +
...
quintileBoxplotOverweight <- ggplot(
  plot_data,
  aes(x = wealth_quintile, y = overweight_prevalence, fill = gdpLevel)
) +
  geom_boxplot(alpha = 0.5) +
  scale_fill_manual(values = c(
    "#06B6D4",
...
################################################################################
## Maps on inequality in obesity and overweight (Figure 2)
################################################################################
...
diffData <- map_data |>
  ...
  mutate(wealth_quintile = case_when(
    wealth_quintile == 1 ~ "Bottom 20%",
    wealth_quintile == 5 ~ "Top 20%"
  )) |>
  group_by(Country, wealth_quintile) |>
  summarise(
    obesityPrev = 100 * wmean(bmicat == 4, w_eq_country),
  ...
  summarise(
    diffObesity = obesityPrev[2] - obesityPrev[1],
...
overweightIneqMap <- ggplot() +
  ...
  geom_polygon(
    data = world_map,
    aes(x = long, y = lat, group = group),
    fill = NA, color = alpha("#404040", 0.6),
    linewidth = 0.001
  ) +
  scale_fill_gradient2(
    low = "#7A3DB8",
...
## Compute summary statistics on the direction of socioeconomic gradients
## For the results text
diffData |>
  group_by(diffOverweight > 0) |>
  summarise(n = n())
diffData |>
  group_by(diffObesity > 0) |>
  summarise(n = n())
